...
#Removing the 48 observations where guests > accomodates. 
...
par(mfrow=c(2,3))
binnedplot( na.omit(train)$price, residuals(log_mod3, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(train)$guests, residuals(log_mod3, type="deviance"),
            nclass = 15, xlab="Number of guests" )
binnedplot( na.omit(train)$min_nights, residuals(log_mod3, type="deviance"),
            nclass = 15, xlab="Minimum length of stay" )
binnedplot( na.omit(train)$revs_month, residuals(log_mod3, type="deviance"),
            nclass = 15, xlab="Reviews per month" )
binnedplot( na.omit(train)$revs_rating, residuals(log_mod3, type="deviance"),
...
```
We decide to take ratings equal to 100 to mean very high review scores. (Top 15%)

```{r}
densityPlot(data$revs_rating)
```
...
test1 <- glm(high_rating ~. -revs_rating-avail-revs_month+I(price^2), family = binomial(link = "logit"), data = data)
summary(test1)

step1 <- step(test1, trace = FALSE)
summary(step1)
...
binnedplot( step1$fitted.values, residuals(step1, type="deviance"),
            nclass = 15, ylab="Average deviance residuals" )

par(mfrow=c(2,3))
binnedplot( na.omit(data)$price, residuals(step1, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(data)$resp_rate, residuals(step1, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(data)$list_count, residuals(step1, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(data)$bathrooms, residuals(step1, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(data)$accommodates, residuals(step1, type="deviance"),
            nclass = 15, xlab="Daily price" )
binnedplot( na.omit(data)$guests, residuals(step1, type="deviance"),
            nclass = 15, xlab="Number of guests" )
binnedplot( na.omit(data)$min_nights, residuals(step1, type="deviance"),
            nclass = 15, xlab="Minimum length of stay" )
binnedplot( na.omit(data)$num_reviews, residuals(step1, type="deviance"),
            nclass = 15, xlab="Number of reviews" )
binnedplot( na.omit(data)$avail_30, residuals(step1, type="deviance"),
            nclass = 15, xlab="Available 30" )
# Doesn't look that great tbh :/ (points supposed to lie within grey lines)
```
