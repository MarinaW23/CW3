...
```{r}
#AirbnbRio[AirbnbRio$host_response_rate==("N/A")]<-NA
AirbnbRio$host_response_rate <- na_if(AirbnbRio$host_response_rate,"N/A")
levels(AirbnbRio$host_response_rate)
summary(AirbnbRio$host_response_rate)
```

```{r}
#plot(AirbnbRio$accommodates,AirbnbRio$guests_included)
...
```
```{r}
apply(is.na(AirbnbRio),2,mean)
mean(apply(is.na(AirbnbRio),1,sum)==0)
source("plot.miss.R")
plot.miss(AirbnbRio)
resp_miss <-is.na(AirbnbRio$host_response_rate)
cor(AirbnbRio[,-c(2:6)],resp_miss,use= "pairwise")
boxplot(AirbnbRio$number_of_reviews~ resp_miss)
#missingness not dep on no. of reviews
boxplot(AirbnbRio$reviews_per_month~ resp_miss)
