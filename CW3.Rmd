...
test=data2[i,] #10% of dataset
train=data2[-i,] #90% of dataset
```
...
#(Almost) Full model
log_mod1 <- glm(avail ~ price + resp_rate + n_hood + prop_type + 
...
#                revs_rating, family=binomial, data=train)
#summary(log_mod2)
...
binnedplot( log_mod3$fitted.values, residuals(log_mod3, type="deviance"),
            nclass = 15, ylab="Average deviance residuals" )

#Continuous predicts vs residuals for log_mod1
par(mfrow=c(2,3))
binnedplot( train$price, residuals(log_mod1, type="deviance"),
...
binnedplot( train$guests, residuals(log_mod1, type="deviance"),
            nclass = 15, xlab="Number of guests" )
binnedplot( train$bathrooms, residuals(log_mod1, type="deviance"),
            nclass = 15, xlab="Number of bathrooms" )
binnedplot( train$min_nights, residuals(log_mod1, type="deviance"),
            nclass = 15, xlab="Minimum length of stay" )
binnedplot( train$revs_month, residuals(log_mod1, type="deviance"),
            nclass = 15, xlab="Reviews per month" )
binnedplot( train$revs_rating, residuals(log_mod1, type="deviance"),
...
## Define the bins
num_bins <- 15
cut_offs <- quantile( log_mod1$fitted.values, 
                      (0:num_bins) / num_bins )

## Store availability status as numeric 0 or 1
avail_numeric <- as.numeric(data$avail)-1
...
for( i in 1:num_bins ){
  ind <- which( log_mod1$fitted.values >= cut_offs[i] & 
                log_mod1$fitted.values <  cut_offs[i+1]  )
  proportion_observed[i]  <- mean( avail_numeric[ind] )
  proportion_predicted[i] <- mean( log_mod1$fitted.values[ind] )
}
...
brier.score(as.numeric(train$avail)-1,log_mod1) #0.1286449 / 0.1029814 (omit)
brier.score(as.numeric(train$avail)-1,log_mod3) #0.1287746 / 0.1064913
...
mis.class(train,train$avail,log_mod1,0.5) #0.1178
mis.class(train,train$avail,log_mod3,0.5) #0.1481
```


```{r,include=F}
#Misclassification rate when cut-off probability is taken to be 0
predicted1 <- as.integer(log_mod1$fitted.values > 0.5)
(tbl <- table(predicted1,train$avail))
mean(predicted1 != train$avail) #11.75% correctly predicted because of imbalance in data

predicted2 <- as.integer(log_mod3$fitted.values > 0.5)
(tbl2 <- table(predicted2,train$avail))
mean(predicted2 != train$avail)

(se1 <- tbl2[2,2]/sum(tbl2[,2])) #sensitivity = true positive rate
(sp1 <- tbl2[1,1]/sum(tbl2[,1])) #specificity = true negative rate

xtabs(~train$avail+predicted1)
1-mean(predicted1[train$avail=="0"])
...
mis.class(test,test$avail,log_mod1,0.5)
mis.class(test,test$avail,log_mod3,0.5)
...
for(i in 1:10){
  predicted <- as.integer(predict(log_mod1,test,type="response") > threshold[i])
...
se=rep(0,10); sp=rep(0,10)
for(i in 1:10){
  predicted <- as.integer(predict(log_mod3,test,type="response") > threshold[i])
  xx=xtabs(~test$avail+predicted)
  se[i]=xx[2,2]/sum(xx[2,])
  sp[i]=xx[1,1]/sum(xx[1,])
}
...
#New rates
mis.class(test,test$avail,log_mod1,0.86)
mis.class(test,test$avail,log_mod3,0.87)
```

Leave-one-out prediction error:
```{r,warning=F}
#Since the response is a binary variable, an appropriate cost function corresponding to misclassification rate is:
cost <- function(r, pi = 0) mean(abs(r-pi) > 0.5)

cv.glm(train, log_mod1, cost)$delta[1]
